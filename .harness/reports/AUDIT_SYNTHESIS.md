...
| 2l | ~~Pozostałe electrical H-2, H-3, H-4, M-1, M-6, L-1, L-3, L-5~~ **DONE 2026-06-30 (commity `7be5dc0` H-3, `1d29ba4` H-4, `f24721e` M-1).** H-2 closure by non-existence (`createDemoCircuitRows` nie istnieje w `circuitRows.ts`, `phase: "N/PE"` nie ma nigdzie). H-3: `buildRcdUpdate` helper w `rcdManagerLogic.ts` porownuje rcdRatedCurrent/rcdResidualCurrent/rcdType przed/po createDefaultSymbolItem; referencja preserved gdy wartosci identyczne. H-4: `validateNoRcdProtection` pomija MCB z rcdSymbolId (niezaleznie czy RCD istnieje); dangling reference obslugiwany przez SEM-007. M-1: `isDistributionBlockSymbol` zawężone do 3 konkretnych fraz ('blok rozdzielczy', 'blok rozgalezny', 'distribution block') + wykluczenie 'rozlacznik'. M-6 (commit `4f43afb`), L-1, L-3, L-5 (B.3-B.7) - zrobione wczesniej. | electrical various | electrical-expert | — |
