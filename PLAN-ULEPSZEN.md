...
| # | Zadanie | Gdzie | Wysiłek |
|---|---|---|---|
| P2-1 | **Zabij nazwę „Pixi".** Rename `DinRailCanvasPixi.tsx` → `DinRailCanvas.tsx` i typ `DinRailCanvasRail` → `DinRailCanvasRail` (nazwa OK) lub `DinRail`. Dotyka ~40 plików (import type). Robić jako 1 mechaniczny PR z pełnym `tsc`+testy po. Wysoka wartość: to centralny komponent. | ~40 plików, punkt startowy `src/components/DinRailCanvasPixi.tsx` | **L** |
