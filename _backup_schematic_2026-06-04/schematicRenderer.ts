...
function drawRcd(ctx: CanvasRenderingContext2D, rcd: SchematicNode, page: PageInfo, hasTopSwitch = false): void {
  ...
  if (rcd.hasNeutralBar) {
    drawGroupNeutralBus(ctx, rcd, page, hasTopSwitch);
  }
}

function drawGroupNeutralBus(
  ctx: CanvasRenderingContext2D,
  rcd: SchematicNode,
  page: PageInfo,
  hasTopSwitch = false
  ...
  // Branch off just below the RCD, completely avoiding the phase marks which are in the middle.
  const branchY = symbolBottomY(rcd.y) + 8;
  const branchEndX = cx + 25; 
  
  // Draw connection dot on the main wire
  drawDot(ctx, cx, branchY, COLORS.wire, 1.8);
  
  // Draw dashed blue line indicating the N pole going to the terminal block
  ctx.save();
  ctx.setLineDash([3, 2]);
  strokeLine(ctx, cx, branchY, branchEndX, branchY, COLORS.n, 1.2);
  ctx.restore();
  
  // Draw terminal block symbol (an open circle)
  ctx.beginPath();
  ctx.arc(branchEndX + 3, branchY, 3, 0, Math.PI * 2);
  ctx.strokeStyle = COLORS.n;
  ctx.lineWidth = 1.2;
  ctx.stroke();
  
  // Label to the right of the terminal block
  const label = rcd.neutralBarLabel || `N (${rcd.designation})`;
  text(ctx, label, branchEndX + 10, branchY - 4, 7.5, COLORS.n, true);
}

function drawChildFromGroupBus(ctx: CanvasRenderingContext2D, child: SchematicNode, page: PageInfo, groupY: number, hasTopSwitch = false): void {
